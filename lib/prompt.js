...
ANONYMITY RULE — STRICTLY ENFORCED: Never use participant names in any generated text, including meetingContext, timeEffectivenessSummary, safeRaceRatio interpretation, crossBehaviouralPatterns, redFlags, and recommendations. Refer to individuals only as "one participant", "a contributor", "the dominant voice", "the facilitator", "a quieter voice", or similar role-based or behaviour-based descriptors. This applies even if names are visible in the transcript.
...
3. Authority figure dominating and dismissing — describe the pattern using role-based descriptors only, never names
...
7. Passive aggressive or passive defensive behaviour — describe instances without naming individuals
...
{
  "meetingContext": "<1 sentence: what this meeting was about — no participant names>",
  "portIn": {
  ...
  "redFlags": ["<pattern — max 20 words, no participant names>"],
  "recommendations": [
    "<actionable recommendation — max 25 words, no participant names>",
    "<actionable recommendation — max 25 words, no participant names>",
    "<actionable recommendation — max 25 words, no participant names>"
  ],
  "behaviourDistribution": {
    ...
    "safeRaceRatio": {
      ...
      "interpretation": "<one sentence — include groupthink flag if Blue — no participant names>"
    },
    "quadrantBreakdown": [
    ...
    "crossBehaviouralPatterns": [
      "<pattern name if detected — max 15 words — no participant names>"
    ],
    "timeEffectivenessSummary": "<2 sentences on contribution balance — Polaris language — no participant names>"
  }
}`;
}
